...
async function installProgressComponent() {
  ...
  try {
    ...
    if (fs.existsSync(targetComponentPath)) {
      ...
      const answer = await askQuestion(
        `${chalk.whiteBright(
          '\nComponent progress already exists.'
      ...
      if (answer.toLowerCase() !== 'y') {
        console.log(
          chalk.blueBright(
            'Skipped progress. To overwrite, run the command again.\n'
          )
        );
        rl.close();
        return;
    ...
    console.log(chalk.greenBright('\nDone! Progress component installed.\n'));

    // install additional dependencies
    exec('npm install @radix-ui/react-progress', (error, stdout, stderr) => {
      if (error) {
    ...
    console.error(
      chalk.redBright('\nError installing progress component:', error)
    );
  } finally {
    rl.close();
  }
